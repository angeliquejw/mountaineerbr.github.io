...
#then the song has changed - get a new album cover	
		if [ "$new_trackid" != "$first_trackid" ];  then
			...
			wget -O   ~/.conky/Aurora/scripts/spotify-api/lastpic_xl $imgurl
			convert ~/.conky/Aurora/scripts/spotify-api/lastpic_xl ~/.conky/Aurora/scripts/spotify-api/lastpic_xl.png
